...
export MC="-j$(nproc)"
export CONTAINER_PACKAGE_URL=mirrors.aliyun.com
export PHP_EXTENSIONS="mysql,mysqli,pdo_mysql,mbstring,bcmath,gd,curl,opcache,sqlsrv,
                      pdo_sqlsrv,zip,pcntl,sockets,soap,odbc,pdo_odbc,wddx,xsl,xmlrpc,
                      imap,mcrypt,redis,memcache,memcached,mongodb,xdebug,swoole,yaf,
                      calendar,dba,exif,gettext,gmp,interbase,intl,odbc,pdo_dblib,
                      PDO_Firebird,pdo_pgsql,pgsql,pspell,recode,shmop,sysvmsg,sysvsem,sysvshm,
                      tidy,wddx"
echo
echo "============================================"
echo "Author                    : felixwang818"
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        installExtensionFromTgz swoole-4.5.1
    else
